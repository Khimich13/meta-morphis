...
def test_should_refresh_meta_no_row(conn):
    assert should_refresh_meta(conn, "pauper") is True

def test_should_refresh_meta_old(conn):
...
def test_should_refresh_meta_fresh(conn):
...
def test_update_meta_timestamp_insert(conn):
...
def test_update_meta_timestamp_update(conn):
    conn.execute(
        "INSERT INTO meta_refresh VALUES (?, ?)",
        ("pauper", 123)
    )
...
def test_load_cached_meta(conn):
    conn.execute("""
...
def test_save_meta_to_cache(conn):
    # old data
    conn.execute("""
